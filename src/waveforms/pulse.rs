...
impl Pulse {
    pub fn set_frequency(&mut self, frequency: f64) {
        self.func.lock().unwrap().set(FREQUENCY_TAG, frequency);
    }

    pub fn set_duty(&mut self, duty: f64) {
        self.func.lock().unwrap().set(DUTY_TAG, duty);
    }

    pub fn stop(&mut self) {
        self.func.lock().unwrap().set(VOLUME_TAG, 0.0);
    }

    pub fn play(&mut self) {
        self.func.lock().unwrap().set(VOLUME_TAG, 0.05);
    }
}
...
impl Iterator for Pulse {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        Some(self.func.lock().unwrap().get_mono() as f32)
    }
}
